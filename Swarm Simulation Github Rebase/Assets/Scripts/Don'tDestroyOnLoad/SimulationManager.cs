...
public class SimulationManager : MonoBehaviour
{
    ...
    public void StartSimulationManager(int startNumber, int endNumber, int numberOfRuns, float pointDistance)
    {
    ...
    IEnumerator ExecuteOnLoad(string sceneNumber)
    {
        ...
        if (SceneManager.GetActiveScene().name == sceneNumber)
        {
            if (currentRun != null)
            {
                ...
                Debug.Log("Inspection time: " + currentRun.inspectionTIme.ToString() + " Simulation time " + currentRun.simulationTime.ToString() + " Collisions: " + currentRun.collisions.ToString());
            }
            if (simulationStack.Count >= 1)
            {
                currentRun = simulationStack.Pop();
                Debug.Log("Starting new simulation run:");
                Debug.Log("Amount of drones: " + currentRun.numberOfDrones.ToString() + " Run number " + currentRun.runNumber.ToString() + " ");
            }
            else
            {
                Application.Quit();
