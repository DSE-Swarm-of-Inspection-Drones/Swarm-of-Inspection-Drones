...
public class StorageManager : MonoBehaviour
{
        ...
        float inspectionTime, float simulatedTime, float numberOfDrones)
    {
        ...
        //string saveLocation = Path.Combine(simulationPath,  "test.txt");
        string temp1 = Path.Combine(simulationPath, folderName0); //"/" + folderName0 + "/" + folderName1 + "/" + simName + ".txt";
        string temp2 = Path.Combine(temp1, folderName1);
        if (!Directory.Exists(temp2))
        {
            Directory.CreateDirectory(temp2);
        }
        string saveLocation = Path.Combine(temp2, simName + ".txt");
