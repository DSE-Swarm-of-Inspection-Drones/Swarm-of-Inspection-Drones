...
public class SurfacePointGenerator : MonoBehaviour   
{
   ...
   public void InspectCube()
   {
    ...
    public class ProjectionPoint
    {
        ...
        public ProjectionPoint(Vector3 coordinateIn, Vector3 projectionDirectionIN)
        {
    ...
    public void addProjectionPoints(Vector3 direction1, Vector3 direction2, Vector3 direction3, int direction2Amount, int direction3Amount)
    {
        ...
        for (var i = 0; i < direction2Amount; i++)
        {
            for (var j = 0; j < direction3Amount; j++)
            {
                ...
                ProjectionPoint projectionPoint = new ProjectionPoint(newPoint, -direction1);
    ...
    public void CreateSurfacePoints(float pointDistance)
    {
        ...
        foreach (ProjectionPoint projectionPoint in projectionPoints)
        {
            ...
            if (Physics.Raycast(transform.TransformPoint(projectionPoint.coordinate), transform.TransformPoint(projectionPoint.projectionDirection), out hit))
            {
        ...
        List<Vector3> surfacePointsToPurge = new List<Vector3>();
        foreach(Vector3 surfacePoint in SurfacePoints)
        {
            foreach (Vector3 surfacePoint2 in SurfacePoints)
            {
                if (surfacePoint != surfacePoint2)
                {
                    if ((surfacePoint2 - surfacePoint).magnitude <= 0.01f)
                    {
        ...
        foreach (Vector3 surfacePointToPurge in surfacePointsToPurge)
        {
